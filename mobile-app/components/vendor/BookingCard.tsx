...
import type { VendorBooking } from '@/utils/bookingHelpers';
import { isPending, isAwaitingAdmin, isOtpRequired } from '@/utils/bookingHelpers';
...
function getStatusBadge(b: VendorBooking): { label: string; bg: string; text: string } {
	...
	if (isPending(b))
		return { label: 'Pending Vendor Response', bg: '#fef9c3', text: '#854d0e' };
	if (b.status === 'completed' || b.status === 'awaiting_review')
		return { label: 'Completed', bg: '#dcfce7', text: '#166534' };
	if (b.status.includes('cancelled') || b.status.includes('rejected'))
		return { label: 'Cancelled / Rejected', bg: '#fee2e2', text: '#991b1b' };
		...
		<View style={[s.card, { backgroundColor: palette.surfaceBg, borderColor: palette.border, borderLeftColor: leftBorder }]}>
			...
			<View style={s.actionRow}>
				{canAR && (
					<>
						<Pressable
							style={[s.actionBtn, s.acceptBtn, { opacity: busy ? 0.6 : 1 }]}
							onPress={onAccept}
							disabled={busy}
						>
							{busy ? (
						...
						<Pressable
							style={[s.actionBtn, s.rejectBtn, { opacity: busy ? 0.6 : 1 }]}
							onPress={onReject}
							disabled={busy}
						>
				...
				{canCancel && (
					<Pressable
						style={[s.actionBtn, s.cancelBtn, { opacity: busy ? 0.6 : 1 }]}
						onPress={onCancel}
						disabled={busy}
					>
			...
			<View style={[s.detailGrid, { borderTopColor: palette.border }]}>
				...
				{booking.eventDate ? (
					<DetailItem
						...
						label="Event Date"
						value={new Date(booking.eventDate).toLocaleDateString('en-IN', {
				...
				<DetailItem
					...
					sub={booking.amount ? `₹${Number(booking.amount).toLocaleString()}` : undefined}
					subColor="#f9a826"
					palette={palette}
				/>
			...
			{canAR && (
				<View style={[s.alertBanner, { backgroundColor: '#fefce8', borderColor: '#fde047' }]}>
					...
					<View style={{ flex: 1 }}>
						...
						<ThemedText style={[s.alertBody, { color: '#a16207' }]}>
			...
			{isAwaitingAdmin(booking) && (
				<View style={[s.alertBanner, { backgroundColor: '#eff6ff', borderColor: '#bfdbfe' }]}>
					...
					<View style={{ flex: 1 }}>
						...
						<ThemedText style={[s.alertBody, { color: '#3b82f6' }]}>
							You have accepted this booking. Waiting for admin to approve.
						</ThemedText>
					</View>
				</View>
			)}
			{needsOtp && (
				<View style={[s.alertBanner, { backgroundColor: '#f5f3ff', borderColor: '#ddd6fe' }]}>
					...
					<View style={{ flex: 1 }}>
						...
						<ThemedText style={[s.alertBody, { color: '#7c3aed' }]}>
							Admin has approved this booking. Ask the customer for their OTP to complete the booking.
						</ThemedText>
					</View>
				</View>
			)}
...
function DetailItem({
	icon, label, value, sub, subColor, palette, wide,
}: {
	icon: string; label: string; value: string;
	sub?: string; subColor?: string; palette: any; wide?: boolean;
}) {
	return (
		<View style={[s.detailItem, wide && s.detailItemWide]}>
			...
			<View style={{ flex: 1 }}>
				...
				{sub ? (
					<ThemedText style={[s.detailSub, { color: subColor ?? palette.subtext }]}>{sub}</ThemedText>
				) : null}
			</View>
		</View>
	);
...
const s = StyleSheet.create({
	card: { borderRadius: 16, borderWidth: 1, borderLeftWidth: 4, overflow: 'hidden' },
