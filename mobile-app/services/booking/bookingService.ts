...
const toAmountLabel = (value: unknown): string => {
  ...
  if (typeof value === "string" && value.trim().length > 0) {
    if (
      value.toLowerCase().includes("rs") ||
      value.toLowerCase().includes("inr")
    ) {
      return value;
    }
...
const toBookingItem = (
  ...
  const idCandidate =
    row.booking_id ?? row.booking_uuid ?? row.id ?? `booking-${index + 1}`;
  ...
  const venueCandidate =
    (typeof row.event_address === "string" &&
    row.event_address.trim().length > 0
      ? row.event_address
      : undefined) ??
    (typeof row.venue === "string" && row.venue.trim().length > 0
      ? row.venue
      : undefined) ??
  ...
  return {
    ...
    vendorName:
      typeof row.vendor_name === "string"
        ...
        : typeof row.business_name === "string"
          ...
          : undefined,
    packageName:
      typeof row.package_name === "string" ? row.package_name : undefined,
    ...
    eventAddress:
      typeof row.event_address === "string" ? row.event_address : undefined,
    specialRequirement:
      typeof row.special_requirement === "string"
...
export const fetchUserBookings = async (
  ...
  return executeApi(async () => {
    ...
    const rows = extractListFromPayload<Record<string, unknown>>(payload, [
      "bookings",
    ]);
...
export const createBooking = async (bookingData: {
  ...
  return executeApi(async () => {
    await apiClient.post(API_ENDPOINTS.booking.create, bookingData);
...
export const submitReview = async (
  ...
  return executeApi(async () => {
    await apiClient.post(
      API_ENDPOINTS.review.submitAuthenticated(bookingId),
      reviewData,
    );
