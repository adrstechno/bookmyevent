    ...
    <View style={styles.vendorCard}>
      ...
      <View style={styles.vendorCardBody}>
        ...
        <Pressable style={styles.viewMoreButton} onPress={onPress}>
          <ThemedText style={styles.viewMoreButtonText}>
            View Details & Book
          </ThemedText>
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) {
          if (!params.serviceName && selectedServiceId) {
            const matched = categories.find(
              (item) => String(item.serviceId) === String(selectedServiceId),
            );
            if (matched) {
        ...
        if (selectedSubserviceId) {
          ...
          if (nextVendors.length === 0 && selectedServiceId) {
      ...
      <View style={[styles.page, { backgroundColor: palette.screenBg }]}>
        ...
        {!loading && vendors.length > 0 && (
          ...
          >
            ...
            {/* Results Count */}
            <ThemedText
              style={[styles.resultsCount, { color: palette.subtext }]}
            >
              ...
              {filteredVendors.map((vendor) => {
                ...
                return (
                  <VendorCard
                    ...
                    imageUri={imageUri}
                    onPress={() =>
                      router.push(`/vendor/${vendor.vendorId}` as never)
                    }
                  />
                );
              })}
